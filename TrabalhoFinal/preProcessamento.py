...
def main():
    ...
    for temporada in temporadas:
        ...
        for episodio in episodios:
            ...
            textoProcessado = textoEpisodio + '\n' + mortesEpisodio + '\n' + falasEpisodio
            
            salvarArquivo(textoProcessado, os.path.join(caminhoTemporadaPreProcessado, episodio))
...
def recuperarFalas(texto):
    try:
        ...
        fimTrecho = re.search(r"([Gg]allery([ ]?Edit)?\n)|JSSnippetsStack|(Image[ ][Gg]allery(Edit)?)|([Pp]romotional[ ][Ii]mages(Edit)?)|([Ss]ee [Aa]lso)|[ ]See:|\Z", trecho).start()
        trecho = trecho[inicioTrecho:fimTrecho]
    except Exception as e:
        return ''

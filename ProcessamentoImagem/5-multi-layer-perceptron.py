...
W1 = tf.Variable(tf.random_normal([768, 400], stddev=0.35))
b1 = tf.Variable(tf.random_normal([400], stddev=0.35))

# weights of the neurons in mid layer
#W_mid = tf.Variable(tf.random_normal([400, 200], stddev=0.35))
#b_mid = tf.Variable(tf.random_normal([200], stddev=0.35))

# weights of the neurons in second layer
W2 = tf.Variable(tf.random_normal([400,10], stddev=0.35))
...
# mid_layer value
#mid_layer = tf.nn.softmax(tf.matmul(hidden_layer, W_mid) + b_mid) 

# output of the network
#y_estimated = tf.nn.softmax(tf.matmul(mid_layer, W2) + b2)
